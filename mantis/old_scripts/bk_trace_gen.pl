...
#  iorate
  ...
  foreach $wssize(@wssizes) {
    foreach $stride(@strides) {
       foreach $iorate(@iorates) {
             print TRACEFILE "$cur_time.0 wctl add mem total=$wssize,wset=$wssize,iorate=$iorate,stride=$stride,etime=$threadtime\n";
             $cur_time += $threadtime;
       }
    }
  }
...
sub GenCPUTrace {
  ...
  if ($use_test == 0) {
    @datatypes     =("int","fp");
    @sleepintervals=(0,1,5,10,50,100,500,1000);
    @matdims    =(50,500,1000);
...
# Matrix sizes
# Sleep intervals
  my($datatype); my($matdim); my($sleepinterval);

  foreach $datatype(@datatypes) {
    for($matdim=0; $matdim<=$#matdims; $matdim++) {
        foreach $sleepinterval(@sleepintervals) {
              print TRACEFILE "$datatype $matdims[$matdim] $iters[$matdim] $sleepinterval\n";
        }
      }
  }
  close(TRACEFILE);
...
# Run CPU calibration -- 1...N CPUs
  ...
  if ($cpu_scale_avail) {
    print SCRIPTFILE "cpufreq-set -c 0 -g userspace\n";
    print SCRIPTFILE "cpufreq-set -c 1 -g userspace\n";

    print SCRIPTFILE "for i in " . $config{"CPU_FREQS_AVAIL"} . "\ndo\n";
    print SCRIPTFILE "  cpufreq-set -f \$i -c 0\n";
    print SCRIPTFILE "  cpufreq-set -f \$i -c 1\n";
  }

  print SCRIPTFILE "\n echo \"**** CPUFREQ = \$i ****\"\n";
  print SCRIPTFILE "\n  echo \"Calibration: Running baseline test\"\n";
  print SCRIPTFILE '  $SCRIPT_DIR/metrics.sh $TRACE_ID 240' . " &\n";
  print SCRIPTFILE "  sleep 180 &\n";
  print SCRIPTFILE "  wait\n";
  ...
  for ($j=0; $j<$config{"CALIB_NUM_CPUS"}; $j++) {
    ...
    print SCRIPTFILE '  $SCRIPT_DIR/metrics.sh $TRACE_ID ' . $cpu_trace_sec*($j+1) . " &\n";
    
    for ($k=0; $k<=$j; $k++) { 
      print SCRIPTFILE "  " . $config{"CALIB_CPU_PROG"} . ' -t $TRACEFILE_DIR/$TRACE_ID-cpu' . " &\n";
    }
    print SCRIPTFILE "  wait\n";
    print SCRIPTFILE "\n  echo \"Calibration: " . ($j+1) . " CPU(s) test complete\"\n";
    print SCRIPTFILE "  sleep 10s\n";
  }
...
# Run memory calibration
  ...
  print SCRIPTFILE "  " . $config{"CALIB_MEM_PROG"} . ' -t $TRACEFILE_DIR/$TRACE_ID-mem' . "\n";
  print SCRIPTFILE "  wait\n";
...
sub GenDAQScript {
  ...
  print SCRIPTFILE "\n# Benchmark function:\n";
  print SCRIPTFILE "# \$1 = name of records, \$2 = function to run on remote machine, \$3 = instrumentation time\n";
  print SCRIPTFILE "benchmark( ) {\n";
...
# print SCRIPTFILE '  if [[ "$temp" =~ "*/calibration.sh" ]]' . "\n";
  print SCRIPTFILE '  if echo "$temp" | egrep "*/calibrate.sh" > /dev/null;' . "\n";
  print SCRIPTFILE "  then\n";
  print SCRIPTFILE "    echo \"Calibrating\"\n";
  ...
  print SCRIPTFILE "  wait\n";
  print SCRIPTFILE "\n  # Copy power measurements to MUT\n";
